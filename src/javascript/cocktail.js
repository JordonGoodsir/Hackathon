  ...
  fetchCocktailJSON().then((cocktail) => {
    ...
    for (item in thisDrink) {
      ...
      if (
        ...
        if (!isNaN(item.slice(-2))) {
          strMeasure = `strMeasure${item.slice(-2)}`;
        }
        let ingredient = document.createElement("p");
        ingredient.textContent =
          thisDrink[strMeasure] === null
            ? `Ingredient: ${thisDrink[item]}`
            : `Ingredient: ${thisDrink[item]} - measure ${thisDrink[strMeasure]}`;
